...
class ResUsers(models.Model):
    ...
    def _auth_oauth_rpc(self, endpoint, access_token):
        print("\n...(Function call )...def _auth_oauth_rpc(res_user)....")
        if self.env['ir.config_parameter'].sudo().get_param('auth_oauth.authorization_header'):
            print("\n......def _auth_oauth_rpc  :::  if self.env['ir.config_parameter'] (res_user)....")
            ...
            print("\n......def _auth_oauth_rpc  ::: else  (res_user)....",response,"--------::: endpoint: ",endpoint,"----:::req.get(endpoint)",requests.get(endpoint))

        if response.ok: # nb: could be a successful failure
            print("\n......def _auth_oauth_rpc  ::: if response.ok  (res_user)... ::: response.json(): ",response.json())
        ...
        if auth_challenge.type == 'bearer' and 'error' in auth_challenge:
            print("\n......def _auth_oauth_rpc  ::: if auth_challenge.type == 'bearer'  (res_user)....")
    ...
    def _auth_oauth_validate(self, provider, access_token):
        print("\n....(Function call )..def _auth_oauth_validate (res_user)....")
        ...
        if oauth_provider.data_endpoint:
            print("\n......def _auth_oauth_validate :::   if oauth_provider.data_endpoint (res_user)....")
        ...
        print("\n......def _auth_oauth_validate :::   subject (res_user)....")
        if not subject:
    ...
    def _generate_signup_values(self, provider, validation, params):
        print("\n...(Function call )...def _auth_oauth_signin(res_user)....")
    ...
    def _auth_oauth_signin(self, provider, validation, params):
        ...
        """
        print("\n......def _auth_oauth_signin    validation (res_user)....",validation)
        oauth_uid = validation['user_id']
        try:
            oauth_user = self.search([("oauth_uid", "=", oauth_uid), ('oauth_provider_id', '=', provider)])
            print("\n......def _auth_oauth_signin    oauth_user (res_user)....", oauth_user)
            if not oauth_user:
                raise AccessDenied()
    ...
    def auth_oauth(self, provider, params):
        ...
        print("\n......def auth_oauth ::: access_token(res_user)....",access_token)
        validation = self._auth_oauth_validate(provider, access_token)
        print("\n......def auth_oauth ::: validation(res_user)....",validation)

        # retrieve and sign in user
        login = self._auth_oauth_signin(provider, validation, params)
        print("\n......def auth_oauth ::: login(res_user)....",login)
        if not login:
            raise AccessDenied()
    ...
    def _check_credentials(self, password, env):
        print("\n...(Function call )...def _check_credentials(res_user)....")
        try:
