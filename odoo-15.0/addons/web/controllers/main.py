...
def _get_login_redirect_url(uid, redirect=None):
    print("\n\n----(Web/main.py ---- def _get_login_redirect_url-----)----::: uid: ",uid)
    ...
    if request.session.uid: # fully logged
        print("\n----(Web/main.py ---- def _get_login_redirect_url---(line 211)----::: if request.session.uid: ")
        return redirect or '/web'

    # partial session (MFA)
    url = request.env(user=uid)['res.users'].browse(uid)._mfa_url()
    if not redirect:
        print("\n----(Web/main.py ---- def _get_login_redirect_url---(line 217)----::: if not redirect: ")
        return url

    parsed = werkzeug.urls.url_parse(url)
    print("\n----(Web/main.py ---- def _get_login_redirect_url---(line 219)----::: parsed: ",parsed)
    qs = parsed.decode_query()
    qs['redirect'] = redirect
    print("\n----(Web/main.py ---- def _get_login_redirect_url---(line 222)----::: qs['redirect']: ",qs['redirect'])
    return parsed.replace(query=werkzeug.urls.url_encode(qs)).to_url()

def login_and_redirect(db, login, key, redirect_url='/web'):
    print("\n\n----(Web/main.py ---- def login_and_redirect-----)----::: db: "
          ,db,"\n-------:::login :",login,"\n-------:::key :",key)
    uid = request.session.authenticate(db, login, key)
    redirect_url = _get_login_redirect_url(uid, redirect_url)
    print("\n----(Web/main.py ---- def login_and_redirect-----)----::: redirect_url: ",redirect_url)
...
class Home(http.Controller):
    ...
    def web_login(self, redirect=None, **kw):
        print("\n ----(Function Called) -- (line 860) ---  def web_login ( Web ) :: ",request.params)
        ensure_db()
        request.params['login_success'] = False
        # print("sssssssssssssssssssssssssssssssssssssssss",request.params)
        if request.httprequest.method == 'GET' and redirect and request.session.uid:
            print("\n ---- (line 865) ----  def web_login( Web ) ::: if request.httprequest.method == 'GET'")
            return request.redirect(redirect)

        if not request.uid:
            print("\n --- (line 869) ----  def web_login( Web ) ::: if not request.uid:")
            request.uid = odoo.SUPERUSER_ID

        values = {k: v for k, v in request.params.items() if k in SIGN_UP_REQUEST_PARAMS}
        # print("                  -=-=-=-=-=-= v a l u e s         ",SIGN_UP_REQUEST_PARAMS)
        # print("                  -=-=-=-=-=-= v a l u e s         ",request.params.items())
        try:
            values['databases'] = http.db_list()
            print("\n ---- (line 877) ---  def web_login( Web ) ::: values['databases']:")
        except odoo.exceptions.AccessDenied:
            values['databases'] = None
            print("\n ---- (line 880) ---  def web_login( Web ) ::: except odoo.exceptions.AccessDenied")

        if request.httprequest.method == 'POST':
            print("\n --- (line 883) ----  def web_login( Web ) ::: if request.httprequest.method == 'POST':")
            old_uid = request.uid
            try:
                uid = request.session.authenticate(request.session.db, request.params['login'], request.params['password'])
                request.params['login_success'] = True
                print("\n --- (line 888) ----  def web_login( Web ) ::: try uid = request.session")
                return request.redirect(self._login_redirect(uid, redirect=redirect))
            except odoo.exceptions.AccessDenied as e:
                print("\n ---- (line 891) ---  def web_login( Web ) ::: except odoo.exceptions.AccessDenied as e")
                request.uid = old_uid
                if e.args == odoo.exceptions.AccessDenied().args:
                    print("\n --- (line 894) ----  def web_login( Web ) ::: if e.args == odoo.exceptions.AccessDenied().args")
                    values['error'] = _("Wrong login/password")

                else:
                    values['error'] = e.args[0]
        else:
            print("\n ---- (line 900) ---  def web_login( Web ) ::: else")
            if 'error' in request.params and request.params.get('error') == 'access':
                values['error'] = _('Only employees can access this database. Please contact the administrator.')
                print("\n... (line 903) ...def web_login  :::  if 'error' in request.params (web/main.py)....")

        # print("-=-=--=-=---=-=-=-=-=-=-\n > values >>>>>",values,"-=-=--=-=---=-=-=-=-=-=-\n >request.session.get('auth_login' >>>>>",request.session.get('auth_login'))
        if 'login' not in values and request.session.get('auth_login'):
            values['login'] = request.session.get('auth_login')
            print("\n ---- (line 900) ---  def web_login( Web ) ::: if 'login' not in values",values['login'])

        if not odoo.tools.config['list_db']:
            values['disable_database_manager'] = True

        response = request.render('web.login', values)
        print("\n ---- (line 900) ---  def web_login( Web ) ::: response", response)
        response.headers['X-Frame-Options'] = 'DENY'
...
class Session(http.Controller):
    ...
    def get_session_info(self):
        print("-----------  def get_session_info  -------------------",request.session)
    ...
    def authenticate(self, db, login, password, base_location=None):
        print("\n\n----(Web/main.py ---- def authenticate-----)----::: db: ",
              db, "\n-------:::login :", login,"\n-------:::password :", password)
